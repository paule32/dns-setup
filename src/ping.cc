...
static int ping(string target)
{
	...
	else 
	{
		...
		if (!hp)
		{
       		...
       		QString("unknown host: %1.").arg(target.c_str()));
			return -1;
		}
	...
	{
		QMessageBox::information(0,"Error","malloc error.");
		return -1;
	}
...
*/
	...
	{
   		QMessageBox::information(0,"Error",
	    QString("Error: %1\nYou are not superuser?").arg(strerror(errno)));
...
int testPing(string ip)
{
	cout << "ping returned " << ping(ip) << endl;
	return 0;
}
